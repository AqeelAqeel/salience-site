...
export function SignInHero({ friend, authing, onSignIn }: Props) {
  ...
  const headline =
    friend.friend_headline ||
    `Hey ${first} — I built you a thing.`;
  const pitch =
    friend.friend_pitch ||
    "A small, private cockpit that reads your recent email, tells you what actually matters, and drafts replies in your voice. Sign in with Google; your emails never leave your Google account except to let the AI read them.";
          ...
          <div>
            ...
            <div className="rise rise-4 mt-10 flex flex-col sm:flex-row gap-3 sm:items-center">
              ...
              <p className="mono text-[11px] text-[var(--fr-text-low)] max-w-xs">
                read-only · we only pull the last ~40 threads · your mail stays in your account
              </p>
            </div>
            ...
            <div className="rise rise-5 mt-14 pt-6 rule-top grid grid-cols-3 gap-6 max-w-lg">
              ...
              <TinyFact
                ...
                value={
                  <span>
                    <span className="text-[var(--fr-text-hi)]">0</span> sent
                  </span>
                }
              />
              <TinyFact
                label="drafts"
                value={<span className="text-[var(--fr-text-hi)]">in your voice</span>}
              />
            </div>
          </div>
